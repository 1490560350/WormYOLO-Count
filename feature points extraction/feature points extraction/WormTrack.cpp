...
WormTrack::WormTrack(string Path, Mat src, Mat src1, int filename, float m_sca, int m_guass, int m_thre, int m_mor, int m_partnum, Point m_A, Point m_B, seg* m_seg, bool& m_ht, float segmentLength, bool showGUI)
{
	path = Path;
	Orignal_img = src;
	image_num = filename;
...
int WormTrack::Analysis()
{
	...
	FindHeadTail(&comp_contours, boundA, boundB, head, tail, comp_contours.size() / 40 > 1 ? comp_contours.size() / 40 : 1, comp_contours.size() / 100 > 1 ? comp_contours.size() / 100 : 1, 0.2, show);
...
void WormTrack::FindHeadTail(vector<Point>* input, vector<Point>& leftside, vector<Point>& rightside, Point &head, Point &tail, const int longvector, const int shortvector, float PercentLength, bool showGUI)

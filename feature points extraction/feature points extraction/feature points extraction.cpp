...
void ProcessImage(int j, const string &Path, seg &segment, Point &prevHead, Point &prevPharynx, Point &m_A, Point &m_B, bool switchht, double &angleSumHead, int partnum, float &segmentLength) {
	...
	WormTrack Track(Path,I, I1, j, 1, guassian, threshold, morpology, partnum, m_A, m_B, &segment, switchht, segmentLeng);
...
int main() {
	...
	segment.head = head;
	segment.tail = tail;
    std::string Path = "E:\\data4\\egl8\\20110920T152233\\out\\";
...
int DrawImage(seg& segment, Mat& I, int partnum, vector<Point> cur)
{
	...
	circle(overlay, segment.head - m_A, 12, Scalar(0, 0, 255), -1, 8, 0);

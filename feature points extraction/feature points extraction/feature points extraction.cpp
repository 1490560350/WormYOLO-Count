...
void ProcessImage(int j, const string &Path, seg &segment, Point &prevHead, Point &prevPharynx, Point &m_A, Point &m_B, bool switchht, double &angleSumHead, int partnum, float &segmentLength, int scaleimg, bool showGUI) {
	...
	WormTrack Track(Path, I, I1, j, scaleimg, guassian, threshold, morpology, partnum, m_A, m_B, &segment, switchht, segmentLeng,showGUI);
...
int main() {
	...
	for (int j = 1; j <= FileNum / 10; ++j) {
		ProcessImage(j, Path, segment, prevHead, prevPharynx, m_A, m_B, 0, angleSumHead, partnum, segmentLength, scaleimg, false);
	}
	...
	for (int j = 1; j <= FileNum / 10; ++j) {
		ProcessImage(j, Path, segment, prevHead, prevPharynx, m_A, m_B, 1, angleSumHead1, partnum, segmentLength, scaleimg, false);
	...
	totaLength = 0;
	int scaleimg1 = static_cast<int>(ceil(200 / segmentLength));
	...
	for (int j = 1; j <= FileNum + 1; ++j) {
		...
		ProcessImage(j, Path, segment, prevHead, prevPharynx, m_A, m_B, currentSwitchht, angleSumHead, partnum, segmentLength, scaleimg, true);
		prevHead = segment.head;
		...
		imwrite(Path + "analysis results\\" + to_string(j) + "_.png", I_color);
...
int DrawImage(seg& segment, Mat& I, int partnum, vector<Point> cur)
{
	...
	circle(overlay, newCenter[12], 2, Scalar(255, 0, 255), -1, 8, 0);

	circle(overlay, segment.head - m_A, 2, Scalar(0, 0, 255), -1, 8, 0);


	circle(overlay, segment.tail - m_A, 2, Scalar(0, 255, 0), -1, 8, 0);
	...
	for (int i = 0; i < cur.size(); i++)
	{
		if (cur[i].x == 0)
		{
			...
			circle(overlay, segment.center[cur[i].y] - m_A, 2, Scalar(255, 255, 0), -1, 8, 0);
		}
		else if (cur[i].x == -1)
		{

			circle(overlay, segment.center[cur[i].y] - m_A, 2, Scalar(0, 255, 255), -1, 8, 0);
		}
		else
		{

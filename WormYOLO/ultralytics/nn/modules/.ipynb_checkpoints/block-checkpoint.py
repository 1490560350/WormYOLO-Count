...
from .conv_utils import get_conv2d, get_bn,conv_bn, conv_bn_relu
...
class SDI(nn.Module):
    def __init__(self, channels):
        ...
        self.convs = nn.ModuleList([DWConv(c, channels[0]) for c in channels])
        #self.convs = nn.ModuleList(
            #[nn.Conv2d(c, channels[0], kernel_size=3, stride=1, padding=1) for c in channels])
    def forward(self, xs):
